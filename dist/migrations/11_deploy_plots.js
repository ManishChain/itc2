...
const PlotFactory = artifacts.require('PlotFactory');
const PlotRegistry = artifacts.require('PlotRegistry');
...
module.exports = async (deployer, network, accounts) => {
    if (enabledFeatures().includes('PLOTS')) {
        ...
        const { factory } = await enteth_migration_utils_1.deployStateMachineSystem(deployer, accounts, GateKeeper, PlotRegistry, PlotFactory, [AdminRoleRegistry, UserRoleRegistry], uiDefinitions);
        for (const plot of DUMMY_DATA) {
            await createPlot(factory, plot);
        }
    }
};
async function createPlot(factory, plot) {
...
//# sourceMappingURL=11_deploy_plots.js.map