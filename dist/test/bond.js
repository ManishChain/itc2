...
const web3_1 = __importDefault(require("web3")); // one dir up, because it is compiled into ./dist/migrations
...
const MILLION = web3_1.default.utils.toBN(10 ** DECIMALS * 1000000);
...
contract('Bond', (accounts) => {
    ...
    before(async function () {
        ...
        const currencyTx = await currencyFactory.createToken(faker_1.default.finance.currencyName(), DECIMALS);
        const currencyAddress = enteth_migration_utils_1.getNewAddressFromEvents(currencyTx, 'TokenCreated');
        // tslint:disable-next-line: no-any
        currency = await Currency.at(currencyAddress);
        // Bonds
        ...
        await enteth_migration_utils_1.timeTravel(3600 * 24);
        ipfsHash = await storeIpfsHash({
            isin: faker_1.default.finance.iban(),
            issuer: faker_1.default.company.companyName(),
        });
        const tx = await bondFactory.createToken(faker_1.default.company.companyName(), 10 ** DECIMALS * 1000, currency.address, 24, 10, 6, DECIMALS, ipfsHash);
        const bondAddress = enteth_migration_utils_1.getNewAddressFromEvents(tx, 'TokenCreated');
        // tslint:disable-next-line: no-any
        bond = await Bond.at(bondAddress);
    });
    ...
    describe('increaseApproval(spender, value)', () => {
        ...
        it('should revert when approval cannot be increased', async function () {
            ...
            await enteth_migration_utils_1.testRevert(bond.increaseApproval(accounts[1], UINTMAX, { from: accounts[0] }));
        });
    });
    ...
    describe('transferWithData(address _to, uint256 _value, bytes _data)', () => {
        it('transfer tokens with data', async () => {
            ...
            assert.equal(balanceBefore.toNumber(), FIVE.toNumber());
            const tx = await bond.transferWithData(accounts[6], THREE, web3_1.default.utils.asciiToHex('some-payment-reference'), {
                from: accounts[5],
            });
            enteth_migration_utils_1.testEvent(tx, 'Transfer', {
                from: accounts[5],
    ...
    describe('transferFromWithData(address _from,address _to,uint256 _value,bytes _data)', () => {
        it('transfer tokens from with data', async () => {
            ...
            const tx = await bond.transferFromWithData(accounts[8], accounts[9], THREE, web3_1.default.utils.asciiToHex('some-payment-reference'));
            enteth_migration_utils_1.testEvent(tx, 'Transfer', {
                from: accounts[8],
                to: accounts[9],
                value: THREE.toString(),
    ...
    describe('edit(string memory name,uint256 parValue,Currency parCurrency,uint256 maturityPeriod,uint256 couponRate,uint256 couponPeriod,uint8 decimals,string memory ipfsFieldContainerHash)', () => {
        ...
        it('can edit the onchain data of a bond', async () => {
            await bond.edit(faker_1.default.company.companyName(), 10 ** DECIMALS * 2000, currency.address, 24, 10, 6, DECIMALS, await bond.getIpfsFieldContainerHash());
        });
        it('can edit the both the onchain and offchain data of a bond', async () => {
            await bond.edit(faker_1.default.company.companyName(), 10 ** DECIMALS * 3000, currency.address, 24, 10, 6, DECIMALS, ipfsHash);
        });
        it('can no longer edit after the launch period is set', async () => {
            await bond.edit(faker_1.default.company.companyName(), 10 ** DECIMALS * 3000, currency.address, 24, 10, 6, DECIMALS, ipfsHash);
            startDate = dayjs_1.default().add(10, 'day').unix();
            await bond.setIssuanceDate(startDate);
            await bond.edit(faker_1.default.company.companyName(), 10 ** DECIMALS * 3000, currency.address, 24, 10, 6, DECIMALS, ipfsHash);
            await bond.launch(startDate);
            await enteth_migration_utils_1.timeTravel(3600 * 24 * 12);
            await enteth_migration_utils_1.testRevert(bond.edit(faker_1.default.company.companyName(), 10 ** DECIMALS * 3000, currency.address, 24, 10, 6, DECIMALS, ipfsHash));
        });
    });
    const couponFixture = [
        [
    ...
    describe('getCoupons()', () => {
        ...
        it('can get a list of some activated coupons', async () => {
            await enteth_migration_utils_1.timeTravel(3600 * 24 * 7 * 52);
